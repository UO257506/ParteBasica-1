...
extern FIRStruct pasobandaBasicFilter;
...
void __attribute__((interrupt, auto_psv)) _ADCInterrupt(void)
{
    ...
    //Filtros
    FIR (NUM_MUESTRA, &filtrada_seno[posicion], &entrada_seno[posicion], &pasobandaBasicFilter); // Se ejecuta el filtro FIR
    FIR (NUM_MUESTRA, &filtrada_trian[posicion], &entrada_trian[posicion], &pasobandaBasicFilter); // Se ejecuta el filtro FIR
    if(posicion<NUM_MUESTRA)
    {
        entrada_seno[posicion]=(ADCBUFB*5)/4095; //Se?al en RB11 (normalizada en Voltios)
        entrada_trian[posicion]=(ADCBUFA*5)/4095; //Se?al en RB10 (normalizada en Voltios)
        posicion++;
...
{   
    ...
    //===== FILTRO
   	FIRDelayInit (&pasobandaBasicFilter); //Inicializamos el Filtro
